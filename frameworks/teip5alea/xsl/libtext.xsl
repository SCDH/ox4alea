...
<!-- XSLT module for the preview: edited text (main text) -->
<!DOCTYPE stylesheet [
    <!ENTITY lre "&#x202a;" >
    <!ENTITY rle "&#x202b;" >
    <!ENTITY pdf "&#x202c;" >
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="l[not(ancestor::head) and descendant::caesura]">
        <tr>
            ...
            <td class="text-col1">
                ...
                <xsl:apply-templates
                    select="node() intersect descendant::caesura[not(ancestor::rdg)]/preceding::node() except scdh:non-lemma-nodes(.)"/>
            ...
            <td>
                ...
                <xsl:apply-templates
                    select="node() intersect descendant::caesura[not(ancestor::rdg)]/following::node() except scdh:non-lemma-nodes(.)"
                />
            </td>
        </tr>
    </xsl:template>
    ...
    <xsl:template match="seg" mode="after-caesura" priority="1">
        ...
        <!-- output nodes that follow caesura -->
        <xsl:apply-templates
            select="node() intersect descendant::caesura/following::node() except scdh:non-lemma-nodes(.)"/>
        <xsl:call-template name="tag-start-end">
            <xsl:with-param name="node" select="."/>
            <xsl:with-param name="type" select="'end'"/>
        </xsl:call-template>
    ...
    <xsl:template name="tag-start-end">
        ...
        <xsl:choose>
            <!-- ornamented parenthesis around verbatim citation from holy text:
                not in verses (poems) -->
            <xsl:when
                test="not($node/ancestor::l) and name($node) eq 'seg' and $node/@type eq 'verbatim-holy' and matches(scdh:language($node), '^ar') and $type eq 'start'">
                <xsl:text>&#xfd3f;</xsl:text>
            </xsl:when>
            <!-- closing ornamented parenthesis -->
            <xsl:when
                test="not($node/ancestor::l) and name($node) eq 'seg' and $node/@type eq 'verbatim-holy' and matches(scdh:language($node), '^ar') and $type eq 'end'">
                <xsl:text>&#xfd3e;</xsl:text>
            </xsl:when>
            <xsl:otherwise/>
        </xsl:choose>
