...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template mode="generate-entries" match="bibl[@xml:id]" as="map(xs:string, xs:string)">
    ...
    <xsl:template mode="label" match="bibl/text()"/>

    <xsl:template mode="label" match="bibl[choice/abbr]" priority="2">
        <xsl:apply-templates mode="label" select="choice/abbr"/>
    </xsl:template>

    <xsl:template mode="label" match="am"/>

    <xsl:template mode="label" match="title">
        <xsl:value-of select="."/>
    </xsl:template>

    <xsl:template mode="label" match="author">
        <xsl:value-of select="."/>
        <xsl:text>: </xsl:text>
    </xsl:template>

    <xsl:template mode="label" match="editor">
        <xsl:value-of select="."/>
        <xsl:text> (Ed.): </xsl:text>
    </xsl:template>

    <xsl:template mode="label" match="year">
        <xsl:text>, </xsl:text>
        <xsl:value-of select="."/>
    </xsl:template>
